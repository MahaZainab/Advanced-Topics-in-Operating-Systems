...
static void *test_submit_thread(void *arg) {
    ...
    printf("\nAll %d jobs submitted. Waiting for completion...\n", t->num_jobs);
    fflush(stdout);

    /* wait for all jobs in this test batch to finish */
    while (1) {
        pthread_mutex_lock(&queue_lock);
        int left = job_count;
        pthread_mutex_unlock(&queue_lock);
    ...
    /* print metrics for this test run */
    printf("\n");
    print_performance(t->num_jobs);

    /* reset counters so next test starts fresh */
    pthread_mutex_lock(&queue_lock);
    reset_metrics();
    pthread_mutex_unlock(&queue_lock);
...
int cmd_quit(int nargs, char **args) {
    ...
    /* use grand total so quit always reports everything submitted this session */
    if (total_submitted > 0)
        print_performance(total_submitted);
    else if (grand_total_submitted > 0)
        printf("Total number of job submitted: %d\n", grand_total_submitted);
    else
