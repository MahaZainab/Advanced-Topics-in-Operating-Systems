 ...
 * priority is optional, defaults to 1 if not given
 */
int cmd_run(int nargs, char **args) {
    ...
    int priority = (nargs == 4) ? atoi(args[3]) : 1;  /* default priority = 1 */
...
/* args passed to the background test thread */
typedef struct {
...
/* background thread that submits jobs for the test command
 * runs independently so the user can still type list while jobs execute */
 ...
 *
 * submits jobs in a background thread so the user can still use
 * list to watch progress while jobs are running
 */
int cmd_test(int nargs, char **args) {
    ...
    /* pack args for the background thread */
    ...
    /* launch submission in background so prompt stays live */
    pthread_t tid;
    pthread_create(&tid, NULL, test_submit_thread, targs);
    pthread_detach(tid);  /* we don't join it - it frees itself */

    return CMD_OK;
}

/*
 * cmd_quit - wait for everything to finish, print stats, exit
 */
int cmd_quit(int nargs, char **args) {
    (void)nargs; (void)args;

    /* wait for any jobs still running or queued to finish */
    while (1) {
        pthread_mutex_lock(&queue_lock);
        int left = job_count;
        pthread_mutex_unlock(&queue_lock);
